...
def XT_MJG_Surface_Surface_ContactArea2(aImarisId):
    ...
    progress_bar2 = ttk.Progressbar(master, orient="horizontal",
                                  mode="determinate", maximum=100, value=0)
    # And a label for it
    label_1 = tk.Label(master, text="Z-Slice Progress Bar ")
    label_2 = tk.Label(master, text="Time Index Progress Bar ")
    ...
    #Progress bar for dendrite segments
            progress_bar['value'] = int((vIndexZ+1)/vDataSize[2]*100) #  % out of 100
            master.update()

        progress_bar2['value'] = int((vTimeIndex+1)/vSizeT*100) #  % out of 100
        master.update()
    master.destroy()
    master.mainloop()
    ...
    if len(vAllSurfaceAreaTotalNumberOfVoxelsStatIndex)>1:
        vAllSurfaceAreaTotalNumberOfVoxelsStat=list(itemgetter(*vAllSurfaceAreaTotalNumberOfVoxelsStatIndex)(vAllSurfaceAreaStatValues))
    else:
    ...
    if len(vSurfaceContactNumberOfVoxelIndex)>1:
        vSurfaceContactIds=list(itemgetter(*vSurfaceContactNumberOfVoxelIndex)(vSurfaceContactStatIds))
        vSurfaceContactVoxels=list(itemgetter(*vSurfaceContactNumberOfVoxelIndex)(vSurfaceContactStatValues))
    else:
    ...
    XT_MJG_Surface_Surface_ContactArea2(aImarisId)
